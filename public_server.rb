...
module ArduinoGateway
  ...
  class PublicServer 
      ...
      def run()
        unless @controller.respond_to?(:submit_public_request) 
          puts "[PublicServer:run] ERROR: controller does not have submit_public_request callback method"            
          return
        end
        ...
        while (client = @server.accept) 
          ...
          # create a new thread to handle each incoming client request
            ...
            @controller.submit_public_request(client_request, client_count)
          end
